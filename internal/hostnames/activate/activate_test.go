...
func TestActivateHostname(t *testing.T) {
	flags.ProjectRef = apitest.RandomProjectRef()

	t.Run("activate custom hostname", func(t *testing.T) {
...
`))
	...
	t.Run("throws error on service unavailable", func(t *testing.T) {
		...
		// Run test
		err := Run(context.Background(), flags.ProjectRef, nil)
		assert.ErrorContains(t, err, "unexpected activate hostname status 503:")
	})
}
