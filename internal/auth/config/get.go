...
// RunVolcengineGet retrieves Auth general config and outputs to stdout.
// If keys is empty, all config is output; otherwise only the specified keys.
func RunVolcengineGet(ctx context.Context, client *volcengine.Client, workspaceID, branchID string, keys []string) error {
	configMap, err := GetAuthConfig(ctx, client, workspaceID, branchID)
	if err != nil {
		return err
	}
	// Filter by --key if specified (case-insensitive substring match)
	if len(keys) > 0 {
...
func GetAuthConfig(ctx context.Context, client *volcengine.Client, workspaceID, branchID string) (map[string]interface{}, error) {
	...
	body, err := access.DoRequest(ctx, http.MethodGet, "/auth/v1/config", nil)
	if err != nil {
		return nil, err
	}
	var configMap map[string]interface{}
	if err := json.Unmarshal(body, &configMap); err != nil {
		return nil, errors.Errorf("failed to decode auth config response: %w", err)
	}
...
func outputConfig(configMap map[string]interface{}) error {
	...
	default:
		// JSON / YAML / TOML formats: output the map directly
